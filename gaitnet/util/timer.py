...
class Timer(ContextDecorator):
    ...
    def __init__(self, logger: logging.Logger, msg: str | None = None, name: str | None = None, log_level: int = logging.INFO):
    ...
    def time_elapsed(self) -> float:
        ...
        """
        return time.perf_counter() - self._start_time  # type: ignore

    @property
    def total_run_time(self) -> float:
        """The number of seconds that elapsed from when the timer started to when it ended."""
        return self._elapsed_time  # type: ignore
    ...
    def get_timer_info(name: str) -> float:
        ...
        return Timer.timing_info.get(name)  # type: ignore