...
def precompute_distance_transform(
    ...
    for b in range(batch_size):
        ...
        if mask_np.sum() == 0:
            ...
            dist, indices = distance_transform_edt(invalid_mask, return_indices=True)  # type: ignore
...
def project_to_valid_terrain(
    ...
    grid, in_bounds = pos_to_idx(torch.concat(
        [
            x.unsqueeze(-1),
            y.unsqueeze(-1),
        ],
    ...
    for b in range(batch_size):
        ...
        if in_bounds[b]:
            ...
            clamped_i = torch.clamp(grid_i[b], 0, height - 1).int().item()
            clamped_j = torch.clamp(grid_j[b], 0, width - 1).int().item()
...
def generate_footstep_action(
    ...
    device = state.device
    terrain_mask = terrain_mask[:, leg, :, :]  # (batch, height, width)
    height, width = terrain_mask.shape[1:]
    ...
    x, y = project_to_valid_terrain(
        x, y, leg, terrain_mask, nearest_i, nearest_j, pos_to_idx
    )
    ...
    leg_one_hot = F.one_hot(torch.full((batch_size,), leg, device=device), num_classes=5).float()
